...
double Dist_two_nodes(const Node &n1, const Node &n2)
{
    double x1[3], x2[3];
    n1.Centroid(x1);
    n2.Centroid(x2);
...
double Cell_diam(const Cell &c)
{
    ...
    for(unsigned i = 0; i < m; i++){
        for(unsigned j = 0; j < m; j++){
            diam = max(diam, Dist_two_nodes(nodes[i], nodes[j]));
        }
    }
    return diam;
}
...
void Mesh_diam(Mesh &m)
{
    double diam = 0.0;
    for(Mesh::iteratorCell icell = m.BeginCell(); icell != m.EndCell(); icell++){
        diam = max(diam, Cell_diam(icell->self()));
    }
